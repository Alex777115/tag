...
class DbConnection:
    ...
    def get_publications(proj_id):
        publication_query = f'''SELECT content FROM publication P, publications_in_project Pp, project Pj
    ...
    def insert_new_tags(tags_collection):
        with conn.cursor() as curs:
            ...
            tags_query = """INSERT INTO tags (project_id, tag, count)
                VALUES (%s, %s, %s)
            ...
            for tag_data in tags_collection:
                ...
                for i in range(0, len(tags_data), 2):
                    tag = tags_data[i]
                    tag_count = tags_data[i + 1]
                    values.append((project_id, tag, tag_count))
...
dbconnection = DbConnection(database='tagcloud_db', user='gastinha', password='Gastinh@', host='localhost')
